...
ADDUSER(IEN,NAME,INITIALS,AC,VC,FILEACC) ;
 ...
 ;
 ; Node .1: piece 1 = verify code change date, piece 2 = HASHED_VC,
 ;          piece 9 = verify code expiration date
 SET $PIECE(^VA(200,IEN,.1),"^",1)=$HOROLOG
 SET $PIECE(^VA(200,IEN,.1),"^",2)=HVC
 SET $PIECE(^VA(200,IEN,.1),"^",9)=$HOROLOG+3650
 ;
 ; Cross-references
